...
var Controller = (function(_Component) {
	...
	Controller.prototype.init = function init(context) {
		if (!this.initialized) {
	...
	Controller.prototype.invokeParentMethod = function invokeParentMethod(methodName) {
		var _instance;
		...
		return (_instance = this.instance).invokeControllerMethod.apply(_instance, [methodName].concat(args));
	};
...
var ArrayAdapter = (function(_DataAdapter) {
	...
	ArrayAdapter.prototype.initInstance = function initInstance(context, instance) {
...
var Repeater = (function(_Container) {
	...
	Repeater.prototype.initInstance = function initInstance(context, instance) {
...
var Instance = (function() {
	...
	Instance.prototype.getControllerMethod = function getControllerMethod(methodName) {
		if (!this.controller)
			throw new Error(
	...
	Instance.prototype.invokeControllerMethod = function invokeControllerMethod(methodName) {
...
var Restate = (function(_PureContainer) {
	inherits(Restate, _PureContainer);

	function Restate() {
		classCallCheck(this, Restate);
		return possibleConstructorReturn(this, _PureContainer.apply(this, arguments));
	}

	Restate.prototype.declareData = function declareData() {
		var _PureContainer$protot;
	...
	Restate.prototype.init = function init() {
		this.container = PureContainer.create({
			type: PureContainer,
			items: this.children || this.items,
			layout: this.layout,
	...
	Restate.prototype.initInstance = function initInstance(context, instance) {
		var bindings = {};
	...
	Restate.prototype.explore = function explore(context, instance) {
	...
	Restate.prototype.prepareData = function prepareData(context, instance) {
		var data = instance.data,
	...
	Restate.prototype.render = function render(context, instance, key) {
...
var RestateStore = (function(_Store) {
	inherits(RestateStore, _Store);

	function RestateStore() {
		classCallCheck(this, RestateStore);
		return possibleConstructorReturn(this, _Store.apply(this, arguments));
	}

	RestateStore.prototype.setParentData = function setParentData(data) {
		var _this3 = this;
	...
	RestateStore.prototype.setItem = function setItem() {
		var _Store$prototype$setI;

		for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
			args[_key] = arguments[_key];
		}

		var changed = (_Store$prototype$setI = _Store.prototype.setItem).call.apply(
			_Store$prototype$setI,
			[this].concat(args)
		);
		if (changed) this.bubble();
		return changed;
	};

	RestateStore.prototype.deleteItem = function deleteItem() {
		var _Store$prototype$dele;

		for (var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
			args[_key2] = arguments[_key2];
		}

		var changed = (_Store$prototype$dele = _Store.prototype.deleteItem).call.apply(
			_Store$prototype$dele,
			[this].concat(args)
		);
		if (changed) this.bubble();
		return changed;
	...
	RestateStore.prototype.bubble = function bubble() {
		var _this4 = this;

		var notified = this.store.batch(function() {
			var data = _this4.getData();
			for (var key in _this4.bindings) {
				var value = data[key];
				if (value === undefined) _this4.store.delete(_this4.bindings[key]);
				else _this4.store.set(_this4.bindings[key], value);
			}
		});

		//in non-detached mode the parent store triggers a new render cycle
		if (!notified && !this.detached) this.store.notify();
	};
...
var Selection = (function(_Component) {
	...
	Selection.prototype.select = function select(store, record, index, options) {
		this.selectMultiple(store, [record], [index], options);
	};

	Selection.prototype.selectMultiple = function selectMultiple(store, records, indexes, options) {
		//abstract
	};
...
var SimpleSelection = (function(_Selection) {
	...
	SimpleSelection.prototype.selectMultiple = function selectMultiple(store, records, index) {
...
var DummySelection = (function(_Selection2) {
	...
	DummySelection.prototype.selectMultiple = function selectMultiple() {
		//dummy
	};
...
var PropertySelection = (function(_Selection) {
	...
	PropertySelection.prototype.selectMultiple = function selectMultiple(store, records, indexes) {
		var _this2 = this;
...
var KeySelection = (function(_Selection) {
	...
	KeySelection.prototype.selectMultiple = function selectMultiple(store, records, indexes) {
		var _this2 = this;
...
var LabelsTopLayout = (function(_Layout) {
	...
	LabelsTopLayout.prototype.init = function init() {
		if (this.vertical && isUndefined(this.columns)) this.columns = 1;

		_Layout.prototype.init.call(this);
	};
	...
	LabelsTopLayout.prototype.render = function render(context, instance, keyPrefix) {
		...
		children.forEach(function(c) {
			...
			if (c.widget.layout && c.widget.layout.useParentLayout && isArray(r.content)) {
				r.content.forEach(function(item) {
					_this2.addItem(state, item);
				});
			} else {
				_this2.addItem(state, r);
			}
		});
		...
		return VDOM$1.createElement(
			"table",
			{ key: keyPrefix, className: CSS.block(baseClass, this.mod) },
			VDOM$1.createElement("tbody", null, state.rows)
		);
	};

	LabelsTopLayout.prototype.addRow = function addRow(state) {
		if (state.labelCells && state.labelCells.length > 0)
			state.rows.push(VDOM$1.createElement("tr", { key: state.rows.length }, state.labelCells));

		if (state.fieldCells && state.fieldCells.length > 0)
			state.rows.push(VDOM$1.createElement("tr", { key: state.rows.length }, state.fieldCells));

		state.labelCells = [];
		state.fieldCells = [];
	};

	LabelsTopLayout.prototype.addItem = function addItem(state, item) {
		if (!state.labelCells || state.labelCells.length + 1 > this.columns) this.addRow(state);

		state.labelCells.push(
			VDOM$1.createElement(
				"td",
				{ className: this.CSS.element(this.baseClass, "label"), key: state.labelCells.length },
		...
		state.fieldCells.push(
			VDOM$1.createElement(
				"td",
				{ className: this.CSS.element(this.baseClass, "field"), key: state.fieldCells.length },
				validContent$1(item)
			)
		);
	};
...
var ContentPlaceholderScope = (function(_PureContainer2) {
	inherits(ContentPlaceholderScope, _PureContainer2);

	function ContentPlaceholderScope() {
		classCallCheck(this, ContentPlaceholderScope);
		return possibleConstructorReturn(this, _PureContainer2.apply(this, arguments));
	}

	ContentPlaceholderScope.prototype.init = function init() {
		_PureContainer2.prototype.init.call(this);

		if (isString(this.name)) this.name = [this.name];
	};

	ContentPlaceholderScope.prototype.explore = function explore(context, instance) {
	...
	ContentPlaceholderScope.prototype.exploreCleanup = function exploreCleanup(context, instance) {
		this.name.forEach(function(name) {
...
var reload = false;
var navigateConfirmationCallback = null;
var permanentNavigateConfirmation = false;
...
var History = (function() {
	...
	History.addNavigateConfirmation = function addNavigateConfirmation(callback) {
...
function startHotAppLoop(module, element, store, widgets, options) {
	var stop = void 0;
	//webpack (HMR)
	if (module.hot) {
		// accept itself
